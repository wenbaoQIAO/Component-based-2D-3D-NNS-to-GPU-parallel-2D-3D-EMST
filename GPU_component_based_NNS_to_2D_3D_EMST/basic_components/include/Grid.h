...
public:
    ...
    void clone(Grid& grid) {
        grid.dist_level = dist_level;
        grid.dual_level = dual_level;
        grid.resize(width, height);
        grid.assign(*this);
    }

    DEVICE_HOST void gpuClone(Grid& grid) {
        grid.dist_level = dist_level;
        grid.dual_level = dual_level;
    ...
    inline void setIdentical(Grid& grid) {
        grid.assign(*this);
    }

    DEVICE_HOST inline void gpuSetIdentical(Grid& grid) {
        gpuCopyDeviceToDevice(grid);
    }
...
//    }
    ...
    //! HW 04/05/15 : add resetValue method on GPU side
...
//! only for host/device specific configuration)
    ...
    DEVICE_HOST Grid& gpuAssign(Grid const& g2) {
#ifdef CUDA_CODE
        cudaMemcpy(_data, g2._data, pitch * height, cudaMemcpyDeviceToDevice);
#else
        std::memcpy(_data, g2._data, pitch * height);
#endif
        return *this;
    ...
    void gpuCopyDeviceToDevice(Grid<Node> & gpuGrid) {
#ifdef CUDA_CODE
        cudaMemcpy2D(gpuGrid._data,
                     gpuGrid.pitch,
                     _data,
    ...
    void gpuCopyDeviceToHostFirst(Grid<Node> & gpuGrid) {
#ifdef CUDA_CODE
        cudaMemcpy2D(_data,
                     pitch,
                     gpuGrid._data,
